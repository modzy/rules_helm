...
def _helm_release_impl(ctx):
    ...
    """
    helm_binary = ctx.toolchains["@com_github_modzy_rules_helm//toolchains/helm:toolchain_type"].helminfo.tool.files.to_list()
    helm_path = helm_binary[0].path
    helm3_binary = ctx.toolchains["@com_github_modzy_rules_helm//toolchains/helm-3:toolchain_type"].helminfo.tool.files.to_list()
    helm3_path = helm3_binary[0].path
    kubectl_binary = ctx.toolchains["@com_github_modzy_rules_helm//toolchains/kubectl:toolchain_type"].kubectlinfo.tool.files.to_list()
...
helm_release = rule(
    ...
    toolchains = [
        "@com_github_modzy_rules_helm//toolchains/helm:toolchain_type",
        "@com_github_modzy_rules_helm//toolchains/helm-3:toolchain_type",
        "@com_github_modzy_rules_helm//toolchains/kubectl:toolchain_type"
    ],
    executable = True,
)
